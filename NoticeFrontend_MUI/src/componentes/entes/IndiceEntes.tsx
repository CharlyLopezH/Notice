...
const IndiceEntes = () => {
  ...
  const [idToolTip,setIdToolTip] = useState(0)
  ...
  function handleEdit(id: number): void {
    alert(`Llamaré al formulario Entes para modificar el ente con Id: ${id}`)
  }

  function handleDelete(id: number): void {
    alert(`Dar de baja el registro: ${id}`);
  }
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // Función para refrescar la lista después de crear un ente
    const handleEnteCreado = () => {
        setMostrarFormulario(false);
        // Aquí podrías actualizar una lista de entes:
        // fetchEntes(); 
    };
              ...
              {data.map((row) => (
                ...
                >
                  ...
                  <TableCell align="center" sx={{ padding: "8px" }}>
                    <Tooltip title={`Update: Id ${idToolTip}`}>
                      <IconButton
                        color="primary"
                        onClick={() => handleEdit(row.id)}
                        onMouseEnter={()=>{setIdToolTip(row.id)}}
                        sx={{
                          transform: "scale(1.5)", // Tamaño original del icono
                          "&:hover": {
                            backgroundColor: "rgba(25, 118, 210, 0.08)",
                            transform: "scale(1.9)", // Efecto hover adicional
                          },
                                margin: "0 4px",
                        }}
                      >
                    ...
                    <Tooltip title={`Borrar ente ${idToolTip}`}> 
                      <IconButton
                        color="error"
                        onClick={() => handleDelete(row.id)}
                        onMouseEnter={()=>{setIdToolTip(row.id)}}
                        sx={{
                          transform: "scale(1.2)", // Tamaño original del icono
                          "&:hover": {
